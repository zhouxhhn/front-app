...
export class Address extends React.Component<any> {
  onSubmit = () => {
    const { form, store, cb, member } = this.props;
    ...
    form.validateFields({ force: true }, async errors => {
      ...
      const regionRes = await store.getRegion();
      const region: any = {};
      if (regionRes && regionRes.length) {
        region.province = regionRes.filter(item => item.id === data.code[0])[0];
        region.city = region.province.children.filter(
          item => item.id === data.code[1]
        )[0];
        region.district = region.city.children.filter(
          item => item.id === data.code[2]
        )[0];
      }
      ...
      try {
        ...
        if (member) {
          const res = await store.submit(member.mobile, { ...data, ...region });
          if (isSuccess(res)) {
